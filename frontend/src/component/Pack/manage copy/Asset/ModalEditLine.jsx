...
const ConfirmProdModal = ({ open, onClose, material, materialName, batch, mapping_id, selectedPlan, selectedLine, selectedGroup, rmfp_id, onSuccess, editorName, currentProdDetails }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const response = await axios.put(`${API_URL}/api/MatOnTrolley/updateProduction/pack`, payload);
...
const ModalEditLine = ({ open, onClose, data, onSuccess }) => {
  ...
  // Alternative implementation for useEffect in ModalEditLine.jsx
  useEffect(() => {
    if (mat) {
      if (mat_name) {
      ...
      // Only check edit history if we have a valid mapping_id
  ...
  // Update to checkEditHistory function in ModalEditLine.jsx
  const checkEditHistory = async () => {
    setIsLoading(true);
    try {
      // Only proceed if mapping_id is a valid value
      if (mapping_id && mapping_id !== 'undefined') {
        ...
        setEditLimitReached(false); // Default to allowing edits when we can't verify
      }
    } catch (error) {
      console.error("Error checking edit history:", error);
      setEditLimitReached(false); // Default to allowing edits on error
    } finally {
      setIsLoading(false);
    }
  };
