...
const ConfirmProdModal = ({ open, onClose, material, materialName, batch, weight_RM, mapping_id, selectedPlan, selectedLine, selectedGroup, rmfp_id, onSuccess, editorName, currentProdDetails, weight }) => {
  ...
  const handleConfirm = async () => {
    ...
    const payload = {
      ...
      weight: weight !== "" && weight != null
        ? parseFloat(weight)
        : (weight_RM !== undefined && weight_RM !== null
          ? parseFloat(weight_RM)
          : null),
      ...
      <Dialog open={open} onClose={(e, reason) => {
        ...
        <DialogContent>
          ...
          <Stack spacing={1}>
            ...
            {weight !== "" && weight != null && (
              <Typography color="rgba(0, 0, 0, 0.6)">น้ำหนัก: {weight} กก.</Typography>
            )}
...
const ModalEditLine = ({ open, onClose, data, onSuccess }) => {
  ...
  const { batch, mat, mat_name, weight_RM, production: currentProduction, mapping_id, rmfp_id, line_name } = data || {};

  useEffect(() => {
    if (weight_RM !== undefined && weight_RM !== null) {
      setWeight(String(weight_RM));
    }
  }, [weight_RM]);
  ...
  useEffect(() => {
    if (mat) {
      ...
      if (mapping_id && mapping_id !== 'undefined') {
        checkEditHistory();
      }
  ...
  }, [mat, mat_name, currentProduction, line_name, mapping_id]);

  const checkEditHistory = async () => {
    setIsLoading(true);
    try {
      if (mapping_id && mapping_id !== 'undefined') {
        const response = await axios.get(`${API_URL}/api/checkEditHistoryOnTrolley/${mapping_id}`);
        ...
        console.warn("mapping_id is undefined or invalid, skipping edit history check");
